...
int main(int argc, char *argv[]) {
  ...
  GtkWidget *cnt = gtk_button_new_with_label("Count button");
  GtkWidget *lbl = gtk_label_new("My label");

  g_signal_connect(btn, "clicked", G_CALLBACK(end_program), NULL);
  g_signal_connect(win, "delete_event", G_CALLBACK(end_program), NULL);
  g_signal_connect(cnt, "clicked", G_CALLBACK(count_button), lbl);

  GtkWidget *tbl = gtk_table_new(2, 2, TRUE);
  // left, right, top, bottom
  gtk_table_attach_defaults(GTK_TABLE(tbl), lbl, 0, 1, 0, 1);
  gtk_table_attach_defaults(GTK_TABLE(tbl), cnt, 1, 2, 0, 1);
